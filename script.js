...
// Инициализация раздела "Экспертиза"
...
function initEsgCalculator() {
  ...
  // Добавляем обработчик для кнопки рейтингов
  const ratingsButton = document.getElementById("ratings-button")
  if (ratingsButton) {
    ratingsButton.addEventListener("click", toggleRatings)
  }
}
...
function loadRatings() {
  ...
  if (!resultDiv || !recommendationsDiv) return
...
function downloadExcel() {
  ...
  if (typeof XLSX === "undefined") {
    alert("Для скачивания отчета необходима библиотека XLSX. Пожалуйста, подключите ее.")
    return
  }
